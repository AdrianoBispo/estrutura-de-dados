# Heap Data Structure

A ****Heap**** is a complete binary tree data structure that satisfies the heap property: for every node, the value of its children is greater than or equal to its own value. Heaps are usually used to implement priority queues, where the smallest (or largest) element is always at the root of the tree.

![min-heap-1.webp](https://media.geeksforgeeks.org/wp-content/uploads/20241105101737867907/min-heap-1.webp)
![min-heap-2.webp](https://media.geeksforgeeks.org/wp-content/uploads/20241105101737995053/min-heap-2.webp)
![max-heap-1.webp](https://media.geeksforgeeks.org/wp-content/uploads/20241105101737567635/max-heap-1.webp)
![max-heap-2.webp](https://media.geeksforgeeks.org/wp-content/uploads/20241105101737719243/max-heap-2.webp)

## ****Basics****

-   [Binary Heap](https://www.geeksforgeeks.org/dsa/binary-heap/)
-   [Applications](https://www.geeksforgeeks.org/dsa/applications-of-heap-data-structure/)

## ****Other Types of Heaps****

-   [Binomial Heap](https://www.geeksforgeeks.org/dsa/binomial-heap-2/)
-   [Fibonacci Heap](https://www.geeksforgeeks.org/dsa/fibonacci-heap-set-1-introduction/)
-   [Leftist Heap](https://www.geeksforgeeks.org/dsa/leftist-tree-leftist-heap/)
-   [K-ary Heap](https://www.geeksforgeeks.org/dsa/k-ary-heap/)

## ****Easy Problems****

-   [Check if an array is Heap?](https://www.geeksforgeeks.org/dsa/how-to-check-if-a-given-array-represents-a-binary-heap/)
-   [K’th Smallest](https://www.geeksforgeeks.org/dsa/kth-smallest-largest-element-in-unsorted-array/) & [K'th Largest](https://www.geeksforgeeks.org/dsa/kth-largest-element-in-an-array/)
-   [Heap Sort](https://www.geeksforgeeks.org/dsa/heap-sort/)
-   [Heap Sort for Decreasing Order](https://www.geeksforgeeks.org/dsa/heap-sort-for-decreasing-order-using-min-heap/)
-   [Height of a Heap with N Nodes](https://www.geeksforgeeks.org/dsa/height-complete-binary-tree-heap-n-nodes/)

## ****Medium Problems****

-   [Nearly Sorted Array](https://www.geeksforgeeks.org/dsa/nearly-sorted-algorithm/)
-   [K Largest Elements](https://www.geeksforgeeks.org/dsa/k-largestor-smallest-elements-in-an-array/) & [K Smallest](https://www.geeksforgeeks.org/dsa/find-k-smallest-elements-in-an-array/)
-   [k Closest Elements](https://www.geeksforgeeks.org/dsa/find-k-closest-elements-given-value/)
-   [Max diff between two m Sized Subsets](https://www.geeksforgeeks.org/dsa/difference-maximum-sum-minimum-sum-n-m-elementsin-review/)
-   [Check if Binary Tree is Heap](https://www.geeksforgeeks.org/dsa/check-if-a-given-binary-tree-is-heap/)
-   [Huffman Coding](https://www.geeksforgeeks.org/dsa/huffman-coding-greedy-algo-3/)
-   [Nodes less than a value in a Min Heap.](https://www.geeksforgeeks.org/dsa/print-all-nodes-less-than-a-value-x-in-a-min-heap/)
-   [Connect n ropes with minimum cost](https://www.geeksforgeeks.org/dsa/connect-n-ropes-minimum-cost/)
-   [K maximum sum combinations from two arrays](https://www.geeksforgeeks.org/dsa/k-maximum-sum-combinations-two-arrays/)[](https://www.geeksforgeeks.org/dsa/median-of-stream-of-integers-running-integers/)
-   [K’th largest in a stream](https://www.geeksforgeeks.org/dsa/kth-largest-element-in-a-stream/)
-   [Largest triplet product in a stream](https://www.geeksforgeeks.org/dsa/largest-triplet-product-stream/)
-   [k most frequent](https://www.geeksforgeeks.org/dsa/find-k-numbers-occurrences-given-array/)
-   [Min Heap to Max Heap](https://www.geeksforgeeks.org/dsa/convert-min-heap-to-max-heap/)
-   [Check for Min-Heap from Level Order](https://www.geeksforgeeks.org/dsa/given-level-order-traversal-binary-tree-check-tree-min-heap/)
-   [BST to Min Heap](https://www.geeksforgeeks.org/dsa/convert-bst-min-heap/)
-   [K Closest Points to the Origin](https://www.geeksforgeeks.org/dsa/find-k-closest-points-to-the-origin/)

## ****Hard Problems****

-   [The Skyline Problem](https://www.geeksforgeeks.org/dsa/the-skyline-problem-using-divide-and-conquer-algorithm/)
-   [Median in a stream](https://www.geeksforgeeks.org/dsa/median-of-stream-of-integers-running-integers/)
-   [K-th Largest Sum Subarray](https://www.geeksforgeeks.org/dsa/k-th-largest-sum-contiguous-subarray/)
-   [Sort by Frequency](https://www.geeksforgeeks.org/dsa/sort-elements-by-frequency/)
-   [Merge k sorted arrays](https://www.geeksforgeeks.org/dsa/merge-k-sorted-arrays/) & [Linked Lists](https://www.geeksforgeeks.org/dsa/merge-k-sorted-linked-lists/)
-   [Range Covering K Sorted Lists](https://www.geeksforgeeks.org/dsa/find-smallest-range-containing-elements-from-k-lists/)
-   [Prim's Minimum Spanning Tree](https://www.geeksforgeeks.org/dsa/prims-minimum-spanning-tree-mst-greedy-algo-5/)
-   [Dijkstra's Shortest Path](https://www.geeksforgeeks.org/dsa/dijkstras-shortest-path-algorithm-greedy-algo-7/)
-   [Sort from different machines](https://www.geeksforgeeks.org/dsa/sort-numbers-stored-on-different-machines/)
-   [Largest Derangement of a Sequence](https://www.geeksforgeeks.org/dsa/largest-derangement-sequence/)
-   [Merge two binary Max Heaps](https://www.geeksforgeeks.org/dsa/merge-two-binary-max-heaps/)
-   [Sum of all between k1’th and k2’th smallest](https://www.geeksforgeeks.org/dsa/sum-elements-k1th-k2th-smallest-elements/)
-   [A data structure with min and max](https://www.geeksforgeeks.org/dsa/a-data-structure-question/)
